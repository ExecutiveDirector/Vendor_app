...
class _InventoryScreenState extends State<InventoryScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      final data = await InventoryApi.list();
      if (mounted)
        setState(() {
          _items = data;
          _loading = false;
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setSt) => Padding(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            Text('Adjust Stock: ${item.productName}',
                style:
                    const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
            Text('Current: ${item.currentStock} units',
  ...
  Future<void> _applyAdjust(
    ...
    try {
      await InventoryApi.adjustStock(item.inventoryId, delta, reason: reason);
      await _load();
      if (mounted) {
  ...
  Future<void> _showMovements(InventoryItem item) async {
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Row(children: [
            Text('KES ${item.sellingPrice.toStringAsFixed(0)}',
                style:
...
class _MovementsSheet extends StatefulWidget {
  ...
  const _MovementsSheet({required this.item, required this.scrollController});
...
class _MovementsSheetState extends State<_MovementsSheet> {
  ...
  Future<void> _load() async {
    try {
      final data = await InventoryApi.movements(widget.item.inventoryId);
      if (mounted)
        setState(() {
          _movements = data;
          _loading = false;
    ...
    return Column(children: [
      ...
      Expanded(
        child: _loading
            ...
            : _movements.isEmpty
                ? const AppEmpty(
                    icon: Icons.history, message: 'No movement history yet.')
                : ListView.separated(
                    ...
                    itemBuilder: (_, i) {
                      ...
                      return ListTile(
                        ...
                        title: Text('${delta > 0 ? '+' : ''}$delta units'),
                        subtitle: Text(m['reason'] ?? 'manual'),
                        trailing: Text(
